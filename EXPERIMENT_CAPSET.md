...
> **实验课题**：Symmetry-Guided Evolutionary Search for Extremal Cap Sets in $\mathbb{F}_3^4$  
> **研究者**：独立 AI 与数学研究者 (AxiomForge Research)  
> **开源代码库**：[Zwf5458-Py/AxiomForge](https://github.com/Zwf5458-Py/AxiomForge)  
> **实验时间**：2026-09-12 11:17:50  

---

## 1. 核心发现与定量结论 (Executive Findings)

在有限域 $\mathbb{F}_3^4$（$3^4 = 81$ 点）的极值组合搜索中，我们对比了**“朴素盲目演化”**与**“代数对称性先验注入演化”**的表现：

| 实验组别 | 先验设计特征 | 最终帽集大小 | 理论最优上限 | 相对提升幅度 | 检验状态 |
| :--- | :--- | :---: | :---: | :---: | :---: |
| **对照组 (Naive)** | 线性加权、无几何先验 | **16** | 20 | 基准线 | 100% 合法 |
| **实验组 (Symmetry)** | 仿射同余、汉明切片、循环群差分 | **20** | 20 | **+4 点 (+25.0%)** | 100% 合法 |

**学术结论**：显式注入代数对称性（尤其是仿射超平面模 3 同余约束 $\sum a_i x_i \equiv c \pmod 3$ 与汉明范数切片）能够极其显著地打破高维组合搜索的“局部收敛平台期”，在仅 40 轮轻量迭代内将集合容量提升了 **25.0%**。
...
def priority(p: tuple, n: int) -> float:
    ...
    parity = sum(p) % 3
    return float(slice_bonus + (parity == 2) * 30.0 + p[0] * 1.78)
```
...
def priority(p: tuple, n: int) -> float:
    ...
    parity = sum(p) % 3
    return float(slice_bonus + (parity == 1) * 30.0 + p[0] * 3.45)
```
...
1. **完全可复现的代码底座**：任何人拉取仓库运行 `python3 experiments/run_ab_experiment.py --dimension 4` 均可在数秒内复现上述数据。
